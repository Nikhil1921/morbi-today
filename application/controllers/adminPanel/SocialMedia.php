...
class SocialMedia extends MY_Controller {
	...
	{
		parent::__construct();
		$this->redirect = admin("socialMedia");
	}
    
	private $name = 'socialMedia';
    private $title = 'social Media';
	...
	public function get()
    {
        $fetch_data = $this->main->make_datatables('social_links_model');
        $sr = $_POST['start'] + 1;
        $data = array();
        ...
        {  
            ...
            $sub_array[] = $sr;
            $sub_array[] = $row->link;
            $sub_array[] = img('assets/images/'.$row->image, '', 'height="50" width="50"');
        ...
        $output = array(  
            ...
            "recordsFiltered"   => $this->main->get_filtered_data('social_links_model'),
            "data"              => $data,
            $csrf_name          => $csrf_hash
        );
    ...
    public function edit($id)
    {
        ...
        $data['url'] = $this->redirect;
        $data['data'] = $this->main->get($this->table, 'id, link, image, counts', ['id' => d_id($id)]);
